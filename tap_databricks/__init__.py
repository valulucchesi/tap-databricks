...
from tap_databricks.sync import DatabricksAuthentication, DatabricksClient, DatabricksSync
...
def create_sync_tasks(config, state, catalog):
    auth = DatabricksAuthentication(config["api_token"])
    client = DatabricksClient(auth)
    sync = DatabricksSync(client, state)
